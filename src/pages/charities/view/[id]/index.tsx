...
import { getCharityById } from 'apiSdk/charities';
import { CharityInterface } from 'interfaces/charity';

function CharityViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<CharityInterface>(
    () => (id ? `/charities/${id}` : null),
    () =>
      getCharityById(id, {
        relations: ['user'],
      }),
            ...
            <FormWrapper wrapperProps={{ gap: 3, p: 4 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('charity', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/charities/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              <EntityImage entity="charity" entityId={id} />
              <List
                w="100%"
                css={{
                  '> li:not(:last-child)': {
