...
import { getCharities } from 'apiSdk/charities';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['organization-owner', 'fundraiser-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CharityInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/charities` : null),
    () => getCharities({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Charities',
      path: isTenantUser ? `/charities/view/${data?.[0]?.id}` : '/charities',
      entity: 'charity',
