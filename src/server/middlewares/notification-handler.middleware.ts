...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'A new fundraiser named {fundraiserName} has been created by {userName}..create': {
    roles: ['organization-owner', 'fundraiser-manager'],
    key: 'fundraiser_created',
    tenantPath: [],
    userPath: [],
  },
  'A donation of {amount} has been made to {fundraiserName} by {userName}..create': {
    roles: ['organization-owner', 'fundraiser-manager'],
    key: 'donation_made',
    tenantPath: [],
    userPath: [],
  },
  'The fundraiser {fundraiserName} has reached its goal of {goalAmount}..update': {
    roles: ['organization-owner', 'fundraiser-manager'],
    key: 'fundraiser_goal_reached',
    tenantPath: [],
    userPath: [],
  },
};
...
const tenantRoles: string[] = ['organization-owner', 'fundraiser-manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.charity.findFirst({
